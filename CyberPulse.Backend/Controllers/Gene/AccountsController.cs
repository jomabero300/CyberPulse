...
public class AccountsController : ControllerBase
{
    ...
    public AccountsController(IUserUnitOfWork usersUnitOfWork, IConfiguration configuration, IMailHelper mailHelper, IWebHostEnvironment env, ICountryUnitOfWork countryUnitOfWork)
    {
    ...
    public async Task<IActionResult> PutAsync(User user)
    {
        try
        {
            ...
            if (result.Succeeded)
            {
                var updatedUser = await _usersUnitOfWork.GetUserAsync(user.Email!);
    ...
    public async Task<IActionResult> ChangePasswordAsync(ChangePasswordDTO model)
    {
        ...
        user.Photo = !string.IsNullOrEmpty(user.Photo) ? user.Photo!.Substring(user.Photo.IndexOf(_configuration["FolderUsers"]!)) : user.Photo;
    ...
    public async Task<IActionResult> ConfirmEmailAsync(string userId, string token)
    {
        ...
        {
            var url = $"{_configuration["UrlBackend"]}{_configuration["DirectoryFolderUsers"]!}";

            if (!string.IsNullOrWhiteSpace(user.Photo) && user.Photo.Contains(url))
            {
                user.Photo = user.Photo.Replace(url,"");
            }
