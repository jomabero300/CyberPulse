...
{
    public static byte[] GenerarPdf(List<Chip> entity)
    {
        ...
        var document = Document.Create(container =>
        {
            ...
            {
                page.Margin(50);
                page.Header().Element(ConstruirCabecera);
                page.Content().Element(ConstruirContenido);
                page.Footer().Element(contruirPie);
            });
        });
        void ConstruirCabecera(IContainer contenedor)
        {
            contenedor.Row(Fila =>
            {
                Fila.RelativeItem().Column(columna =>
                {
                    ...
                    columna.Item().Height(2, Unit.Centimetre).Image(imagePath);
                    columna.Item().Scale(tamañoDeLetra).Text("Carrera 20 No. 28-163");
                    columna.Item().Scale(tamañoDeLetra).Text("Nit: 899.999.034-1");
                    columna.Item().Text("");
                    columna.Item().Scale(tamañoDeLetra).Text($"Fecha: {DateTime.UtcNow.ToString("dd/mm/yyyy")}");
                    columna.Item().Text("");
                    columna.Item().Scale(tamañoDeLetra).Text("SENA Regional Arauca");
                });
            });
        }
        ...
        void contruirPie(IContainer contenedor)
        {
            var tamañoDeLetra = 0.6f;
            contenedor.Column(columna =>
            {
                ...
                {
                    row.Spacing(5);
                    row.RelativeItem().Scale(tamañoDeLetra).Text("Dirección General/Regional/Centro").Bold();
                    row.RelativeItem().Scale(tamañoDeLetra).Text("Dirección Carrera 20 No. 28-163, Ciudad Arauca. - PBX 607 8856339").Bold();
                });
                columna.Item().AlignCenter().PaddingTop(5).Scale(tamañoDeLetra).Text(text =>
                {
                    text.Span("Página ").Bold();
