...
public partial class TrainingProgramIndex
{
    ...
    private MudTable<TrainingProgram> table = new();
    private readonly int[] pageSizeOptions = { 10, 25, 50, int.MaxValue };
    private int totalRecords = 0;
    private bool loading;
    private const string baseUrl = "api/trainingprograms";
    ...
    private async Task<TableData<TrainingProgram>> LoadListAsync(TableState state, CancellationToken cancellationToken)
    {
        ...
        var responseHttp = await repository.GetAsync<List<TrainingProgram>>(url);
        ...
        if (responseHttp.Error)
        {
            ...
            return new TableData<TrainingProgram> { Items = [], TotalItems = 0 };
        }
        ...
        return new TableData<TrainingProgram>
        {
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = await DialogService.ShowAsync<TrainingProgramEdit>(
                $"{Localizer["Edit"]} {Localizer["TrainingProgram"]}",
                parameters,
                options);
        }
        else
        {
            dialog = await DialogService.ShowAsync<TrainingProgramCreate>($"{Localizer["New"]} {Localizer["TrainingProgram"]}", options);
        }
    ...
    private async Task DeleteAsync(TrainingProgram entity)
    {
        var parameters = new DialogParameters
        {
