...
public partial class CategoryIndex
{
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = await DialogService.ShowAsync<CategoryEdit>(
                $"{Localizer["Edit"]} {Localizer["Category"]}",
                parameters,
                options);
        }
        else
    ...
    private async Task DeleteAsync(Category entity)
    {
        var parameters = new DialogParameters
        {
            { "Message", string.Format(Localizer["DeleteConfirm"], Localizer["Category"], entity.Id) }
        };
