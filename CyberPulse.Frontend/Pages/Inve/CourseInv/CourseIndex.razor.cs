...
public partial class CourseIndex
{
    ...
    private MudTable<Course> table = new();
    ...
    private async Task<TableData<Course>> LoadListAsync(TableState state, CancellationToken cancellationToken)
    {
        ...
        var responseHttp = await repository.GetAsync<List<Course>>(url);
        ...
        if (responseHttp.Error)
        {
            ...
            return new TableData<Course> { Items = [], TotalItems = 0 };
        }
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = await DialogService.ShowAsync<CourseEdit>(
                $"{Localizer["Edit"]} {Localizer["Course"]}",
                parameters,
                options);
        }
        else
        {
            dialog = await DialogService.ShowAsync<CourseCreate>($"{Localizer["New"]} {Localizer["Course"]}", options);
        }
    ...
    private async Task DeleteAsync(Course entity)
    {
        var parameters = new DialogParameters
        {
            { "Message", string.Format(Localizer["DeleteConfirm"], Localizer["Course"], entity.Name) }
        };
