...
public partial class BudgetIndex
{
    ...
    private MudTable<Budget> table = new();
    ...
    private async Task<TableData<Budget>> LoadListAsync(TableState state, CancellationToken cancellationToken)
    {
        ...
        var responseHttp = await repository.GetAsync<List<Budget>>(url);
        ...
        if (responseHttp.Error)
        {
            ...
            return new TableData<Budget> { Items = [], TotalItems = 0 };
        }
    ...
    private async Task DeleteAsync(Budget entity)
    {
        var parameters = new DialogParameters
        {
