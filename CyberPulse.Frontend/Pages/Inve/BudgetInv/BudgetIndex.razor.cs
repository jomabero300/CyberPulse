...
{
    private List<Budget1DTO>? budgets { get; set; }
    private MudTable<Budget1DTO> table = new();
    ...
    private async Task<TableData<Budget1DTO>> LoadListAsync(TableState state, CancellationToken cancellationToken)
    {
        ...
        var responseHttp = await repository.GetAsync<List<Budget1DTO>>(url);
        ...
        if (responseHttp.Error)
        {
            ...
            return new TableData<Budget1DTO> { Items = [], TotalItems = 0 };
        }
        ...
        return new TableData<Budget1DTO>
        {
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = await DialogService.ShowAsync<BudgetEdit>(
        ...
        {
            dialog = await DialogService.ShowAsync<BudgetCreate>($"{Localizer["New"]} {Localizer["Budget"]}", options);
        }
    ...
    private async Task DeleteAsync(Budget1DTO entity)
    {
        var parameters = new DialogParameters
        {
