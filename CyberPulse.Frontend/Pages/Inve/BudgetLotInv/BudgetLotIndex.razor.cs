...
{
    private List<BudgetLot>? budgetLots { get; set; }
    private MudTable<BudgetLot> table = new();
    ...
    private async Task<TableData<BudgetLot>> LoadListAsync(TableState state, CancellationToken cancellationToken)
    {
        ...
        var responseHttp = await repository.GetAsync<List<BudgetLot>>(url);
        ...
        if (responseHttp.Error)
        {
            ...
            return new TableData<BudgetLot> { Items = [], TotalItems = 0 };
        }
        ...
        return new TableData<BudgetLot>
        {
    ...
    private async Task DeleteAsync(BudgetLot entity)
    {
        var parameters = new DialogParameters
        {
